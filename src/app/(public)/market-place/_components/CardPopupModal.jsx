...
export function CardModal({ type, onClose, card }) {
  const {
    ...
    defaultValues: {
      ...
      price: card?.price || 0,
      grade: card?.wishGrade || '',
      genre: card?.wishGenre || '',
      description: card?.description || '',
    },
  });

  const subject = type === 'sell' ? '나의 포토카드 판매하기' : '수정하기';
  const cardImage = card?.imageUrl || img_card;
    ...
    >
      <div
        className={cn(
          ...
          'min-h-screen h-full py-0 px-0 rounded-none',
          'sm:min-h-0 sm:max-h-[90vh] sm:bg-gray-500 py-3.75 sm:px-2.5',
          'sm:rounded-t-[2px] sm:rounded-b-none sm:border-b-0',
          'md:max-w-290 md:max-h-250 md:py-15 md:px-30',
        ...
        <div className="w-full max-w-full mx-auto sm:max-w-none">
          <header className="px-6 py-4">
            <div className="flex justify-between items-center">
              ...
              <h1 className="text-[1.25rem] font-br text-white tracking-[-0.6px] text-center sm:text-gray-300 sm:text-base sm:text-left sm:tracking-[-0.72px] md:text-2xl">
                {subject}
              </h1>
              <button
              ...
              <section className="flex justify-between">
                <div className="flex flex-col w-full gap-4 mt-4 sm:flex-row sm:gap-5 md:gap-10">
                  <div className="relative aspect-video bg-gray-900 rounded-lg object-cover sm:min-w-20 sm:max-w-85.5 sm:max-h-[16.0313rem] md:max-w-110 md:max-h-82.5 sm:flex-1">
                    <Image src={cardImage} alt="기본 이미지" fill />
                  </div>
